    ...
    return (
      theme == 'dark' ?
        <button onClick={() => setTheme('light')} aria-label="Light">
          <SunIcon className="h-[23px] w-[23px] text-neutral-400 hover:text-neutral-200 transition-all" />
        </button>
        :
        <button onClick={() => setTheme('dark')} aria-label="Dark">
          <MoonIcon className="h-[23px] w-[23px] text-neutral-500 hover:text-neutral-700 transition-all" />
        </button>
    )
  }
